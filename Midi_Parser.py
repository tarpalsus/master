...
class MidiParser():
    ...
    def concat_instrument(self, instrument_to_find='piano'):
        out_notes = []
        for instrument in self.midi_file.instruments:
            if instrument_to_find in instrument.name.lower():
                out_notes += instrument.notes
        out_notes = sorted(out_notes, key=lambda x: x.start)
        self.midi_file.instruments[0].notes = out_notes
